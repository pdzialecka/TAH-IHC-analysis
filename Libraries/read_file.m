...
elseif strcmpi(ext,'.avi')
    ...
    if v.Duration*v.FrameRate < sframe
        ...
        num2read = int32(min(num2read,v.Duration*v.FrameRate-sframe+1));  % int needed; @pdzialecka
        Y1 = readFrame(v);
%         tic; Y1 = read(v,1); toc;
        if size(Y1,3) > 1; Y1 = rgb2gray(Y1); end  % convert RGB with idential channels to grayscale; @pdzialecka
        ...
        while hasFrame(v)  && (i - sframe + 1 < num2read)
            video = readFrame(v);
            if size(video,3) > 1; video = rgb2gray(video); end % @pdzialecka
